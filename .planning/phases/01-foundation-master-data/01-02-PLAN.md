...
must_haves:
  truths:
    ...
    - "PostgreSQL audit triggers fire on INSERT/UPDATE/DELETE for master data tables and capture userId via SET LOCAL app.current_user_id"
  artifacts:
    ...
    - path: "src/app/(auth)/login/page.tsx"
      ...
      contains: "signIn"
    - path: "src/server/db/triggers.sql"
...
<task type="auto">
  ...
  **1. Create `src/server/api/trpc.ts`** — tRPC initialization with context:
  ...
  export const protectedProcedure = t.procedure.use(async (opts) => {
    ...
    // Set the current user ID in PostgreSQL session variable for audit triggers
    // This allows the audit_trigger_func() to capture WHO made each change
    await opts.ctx.db.execute(
      sql`SELECT set_config('app.current_user_id', ${opts.ctx.session.user.id}, true)`
    );
    return opts.next({
      ctx: {
...
<task type="auto">
  ...
  <action>
  **1. Create `src/server/db/triggers.sql`** — PostgreSQL audit trigger function:
  ...
  -- Reads app.current_user_id session variable set by application layer via SET LOCAL
  CREATE OR REPLACE FUNCTION audit_trigger_func()
  RETURNS TRIGGER AS $$
  DECLARE
    _user_id text;
  BEGIN
    -- Read the current user ID from the session variable (set by tRPC protectedProcedure)
    _user_id := current_setting('app.current_user_id', true);

    IF TG_OP = 'INSERT' THEN
      INSERT INTO audit_log (table_name, record_id, action, user_id, changed_data, created_at)
      VALUES (TG_TABLE_NAME, NEW.id::text, 'INSERT', _user_id, row_to_json(NEW)::text, NOW());
      RETURN NEW;
    ELSIF TG_OP = 'UPDATE' THEN
      INSERT INTO audit_log (table_name, record_id, action, user_id, changed_data, previous_data, created_at)
      VALUES (TG_TABLE_NAME, NEW.id::text, 'UPDATE', _user_id, row_to_json(NEW)::text, row_to_json(OLD)::text, NOW());
      RETURN NEW;
    ELSIF TG_OP = 'DELETE' THEN
      INSERT INTO audit_log (table_name, record_id, action, user_id, previous_data, created_at)
      VALUES (TG_TABLE_NAME, OLD.id::text, 'DELETE', _user_id, row_to_json(OLD)::text, NOW());
      RETURN OLD;
    END IF;
  END;
  $$ LANGUAGE plpgsql;
  ...
  async function main() {
    ...
    try {
      const triggerSql = readFileSync(
        join(process.cwd(), "src/server/db/triggers.sql"),
        "utf-8"
      );
  ...
  **4. Verify the seed script runs:**
  ...
  <verify>
  - `src/server/db/triggers.sql` exists and contains audit_trigger_func
